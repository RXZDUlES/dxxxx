...
-- ============================================================
-- RXZ PROTECTION / TECHNICAL WRAPPER (stable, memory-safe)
-- ============================================================
do
...
-- 4. OPTIMIZATION MARKER
local LPH_NO_VIRTUALIZE = function(f) return f end

-- 5. AUTOMATIC CLEANUP (teardown)
...
_G._RXZ_SECURE = _secureWrap
_G._RXZ_NOVIRT = LPH_NO_VIRTUALIZE
...
if type(protect_gui) == "function" then
    pcall(function()
        ...
        for _, g in ipairs(_cg:GetChildren()) do
            if g:IsA("ScreenGui") then pcall(function() protect_gui(g) end) end
        end
    end)
end

-- 9. PROPERTY INTERCEPTION
local _xr_orig
if type(hookmetamethod) == "function" then
    pcall(function()
        _xr_orig = hookmetamethod(game, "__index", _secureWrap(function(self, key)
            if key == "LocalTransparencyModifier" and self and self:IsA("BasePart") then
                -- extension point
            end
...
-- 10. EXPOSED RESET
_G.MyScript_Reset = _secureWrap(function()
    pcall(function() _doTeardown() end)
end)

print("[RXZ] Protection structure loaded successfully.")
end
-- ============================================================
...
local function playIntroGUI()
    ...
    local subtitle = createText("RXZ HUB - PREMIUM", 14, 0.55)

    local function fadeIn(object, property)
        local tween = TweenService:Create(object, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {[property] = 0})
        tween:Play(); return tween
    end
    local function fadeOut(object, property)
        local tween = TweenService:Create(object, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {[property] = 1})
        tween:Play(); return tween
    end

    TweenService:Create(shade, TweenInfo.new(0.4), {BackgroundTransparency = 0.15}):Play()
    task.wait(0.5)
    ...
    TweenService:Create(shade, TweenInfo.new(0.8), {BackgroundTransparency = 1}):Play()
    task.wait(0.8)
...
local Steal={AutoStealEnabled=false,StealRadius=63,StealDuration=1.3,Data={}}
...
local function resetAllSettings()
    ...
    Steal.AutoStealEnabled=false;Steal.StealRadius=63;Steal.StealDuration=1.3
    guiTransparencyEnabled=false;mobileButtonsEnabled=true;mobileButtonsSize=110
...
local function createStealBar()
    for _,n in ipairs({"MoveeStealBar","RXZ_StatusUI"}) do
    ...
    stealGui.Name="RXZ_StatusUI";stealGui.ResetOnSpawn=false;stealGui.IgnoreGuiInset=true
    stealGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling;stealGui.DisplayOrder=8
    do
        local ok=false
        if gethui then ok=pcall(function() stealGui.Parent=gethui() end)
        elseif syn and syn.protect_gui then ok=pcall(function() syn.protect_gui(stealGui);stealGui.Parent=game:GetService("CoreGui") end) end
        if not ok then stealGui.Parent=LP:WaitForChild("PlayerGui") end
    end

    local CBG    = Color3.fromRGB(0,0,0)
    local CSTROKE= Color3.fromRGB(255,255,255)
    local CGREY  = Color3.fromRGB(170,170,170)
    local CTRACK = Color3.fromRGB(24,24,24)
    local CDOTBG = Color3.fromRGB(40,40,40)
    local WHITE  = Color3.fromRGB(255,255,255)
    ...
    main.Active=true
    main.Parent=stealGui
    stealBarFrame=main
    Instance.new("UICorner",main).CornerRadius=UDim.new(0,14)
    local stroke=Instance.new("UIStroke",main);stroke.Color=CSTROKE;stroke.Thickness=1.5;stroke.Transparency=0.35

    local dotBg=Instance.new("Frame",main)
    dotBg.Size=UDim2.fromOffset(20,20);dotBg.Position=UDim2.fromOffset(10,7)
    dotBg.BackgroundColor3=CDOTBG;dotBg.BorderSizePixel=0
    Instance.new("UICorner",dotBg).CornerRadius=UDim.new(1,0)
    local dot=Instance.new("Frame",dotBg)
    dot.Size=UDim2.fromOffset(8,8);dot.AnchorPoint=Vector2.new(0.5,0.5);dot.Position=UDim2.fromScale(0.5,0.5)
    dot.BackgroundColor3=WHITE;dot.BorderSizePixel=0
    Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)

    local pctLbl=Instance.new("TextLabel",main)
    pctLbl.BackgroundTransparency=1;pctLbl.Position=UDim2.fromOffset(36,4);pctLbl.Size=UDim2.fromOffset(100,22)
    pctLbl.FontFace=ITALIC;pctLbl.TextSize=18;pctLbl.TextColor3=WHITE
    pctLbl.TextXAlignment=Enum.TextXAlignment.Left;pctLbl.Text="0%"

    local radiusLbl=Instance.new("TextLabel",main)
    radiusLbl.BackgroundTransparency=1;radiusLbl.AnchorPoint=Vector2.new(1,0)
    radiusLbl.Position=UDim2.new(1,-10,0,4);radiusLbl.Size=UDim2.fromOffset(140,22)
    radiusLbl.FontFace=ITALIC;radiusLbl.TextSize=16;radiusLbl.TextColor3=WHITE
    radiusLbl.TextXAlignment=Enum.TextXAlignment.Right
    radiusLbl.Text="Radius: "..tostring(Steal.StealRadius)

    local infoLbl=Instance.new("TextLabel",main)
    infoLbl.BackgroundTransparency=1;infoLbl.Position=UDim2.fromOffset(0,28);infoLbl.Size=UDim2.new(1,0,0,16)
    infoLbl.Font=Enum.Font.GothamBold;infoLbl.TextSize=11;infoLbl.TextColor3=CGREY
    infoLbl.Text="FPS: 0  -  PING: 0ms  -  RXZ HUB"

    local frameCount,timeAccum,fps=0,0,0
    RunService.RenderStepped:Connect(function(delta)
        if not (infoLbl and infoLbl.Parent) then return end
        frameCount=frameCount+1;timeAccum=timeAccum+delta
        if timeAccum>=0.5 then
            fps=math.floor(frameCount/timeAccum+0.5);frameCount=0;timeAccum=0
            local ping=0
            pcall(function() ping=math.floor(LP:GetNetworkPing()*1000+0.5) end)
            infoLbl.Text=("FPS: %d  -  PING: %dms  -  RXZ HUB"):format(fps,ping)
            radiusLbl.Text="Radius: "..tostring(Steal.StealRadius)
        end
    end)

    local track=Instance.new("Frame",main)
    track.Position=UDim2.new(0,10,1,-20);track.Size=UDim2.new(1,-20,0,14)
    track.BackgroundColor3=CTRACK;track.BorderSizePixel=0;track.ClipsDescendants=true
    Instance.new("UICorner",track).CornerRadius=UDim.new(1,0)

    local fill=Instance.new("Frame",track)
    fill.Size=UDim2.fromScale(0,1);fill.BackgroundColor3=WHITE;fill.BorderSizePixel=0
    ...
    task.spawn(function()
        while fill and fill.Parent do
            ...
            if Steal.AutoStealEnabled then
                ...
                grad.Offset=Vector2.new(math.sin(now*3)*0.5,0)
                pctLbl.Text=math.floor(pct*100).."%"
                dot.BackgroundColor3=(pct>0) and WHITE or Color3.fromRGB(90,90,90)
            else
                fill.Size=UDim2.fromScale(0,1);pctLbl.Text="0%"
    ...
    local dragging2,dragStart2,dragStartPos2=false,nil,nil
    main.InputBegan:Connect(function(input)
        if uiLocked then return end
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
            dragging2=true;dragStart2=input.Position;dragStartPos2=main.Position
    ...
    UIS.InputChanged:Connect(function(input)
        ...
        if dragging2 and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then
            ...
            main.Position=UDim2.new(dragStartPos2.X.Scale,dragStartPos2.X.Offset+delta.X,dragStartPos2.Y.Scale,dragStartPos2.Y.Offset+delta.Y)
        end
    end)
end
createStealBar()
...
do
    local UserInputService = UIS or game:GetService("UserInputService")
    local Player = LP or game:GetService("Players").LocalPlayer
    ...
    UserInputService.JumpRequest:Connect(function()
        isJumping = true
        task.delay(0.25, function() isJumping = false end)
    end)
    ...
    RunService.Heartbeat:Connect(function()
        ...
        local yv = math.abs(HRP.AssemblyLinearVelocity.Y)
        if yv > 3 then
            isJumping = true
        elseif yv < 1 and isJumping then
            isJumping = false
        end
    end)

    RunService.Heartbeat:Connect(function()
        if not Humanoid or not HRP or not HRP.Parent then return end
        if Humanoid.Health <= 0 then return end

        if Humanoid.WalkSpeed ~= activeSpeedValue then
