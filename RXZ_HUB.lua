...
task.spawn(function()
    ...
    box.AnchorPoint=Vector2.new(1,0.5); box.Position=UDim2.new(1,-40,0.5,0)
    ...
    line.AnchorPoint=Vector2.new(1,0); line.Position=UDim2.new(1,0,0,58); line.Size=UDim2.new(0,0,0,2)
    ...
    sub.TextSize=14; sub.Font=Enum.Font.GothamBold; sub.TextXAlignment=Enum.TextXAlignment.Right; sub.TextTransparency=1
    ...
    dc.TextSize=15; dc.Font=Enum.Font.GothamBold; dc.TextXAlignment=Enum.TextXAlignment.Right; dc.TextTransparency=1
    ...
    tip.TextSize=12; tip.Font=Enum.Font.Gotham; tip.TextXAlignment=Enum.TextXAlignment.Right; tip.TextTransparency=1
...
RunService.RenderStepped:Connect(function()
    ...
    -- PRE-ROUND FREEZE (Night Duels behaviour): while the game holds you in place
    -- (5s countdown before the round starts) the script must NOT push you around.
    if RXZ.isFrozen() then
...
-- ============================================================
-- AUTO MEDUSA (engine + optional on-screen button)
-- ============================================================
RXZ.autoMedusaBtn=false
RXZ.autoMedusaOn=false
RXZ.amConn=nil; RXZ.amCircle=nil; RXZ.amBusy=false
RXZ.AM_RANGE=9.5; RXZ.AM_CD=0.12
RXZ.findMedusaTool=function()
    local ch=LP.Character; local bp=LP:FindFirstChildOfClass("Backpack")
    local function search(c)
        if not c then return nil end
        for _,t in ipairs(c:GetChildren()) do
            if t:IsA("Tool") then
                local n=t.Name:lower()
                if n:find("medusa") or n:find("head") then return t end
            end
        end
        return nil
    end
    return search(ch) or search(bp)
end
RXZ.unequipMedusa=function()
    local ch=LP.Character; local hum=ch and ch:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local med=RXZ.findMedusaTool()
    if med and med.Parent==ch then pcall(function() hum:UnequipTools() end) end
end
RXZ.enemyInRange=function()
    local ch=LP.Character; local root=ch and ch:FindFirstChild("HumanoidRootPart"); if not root then return false end
    for _,o in ipairs(Players:GetPlayers()) do
        if o~=LP and o.Character then
            local r2=o.Character:FindFirstChild("HumanoidRootPart")
            local h2=o.Character:FindFirstChildOfClass("Humanoid")
            if r2 and h2 and h2.Health>0 and (root.Position-r2.Position).Magnitude<=RXZ.AM_RANGE then return true end
        end
    end
    return false
end
RXZ.startAutoMedusa=function()
    RXZ.autoMedusaOn=true
    if RXZ.amConn then RXZ.amConn:Disconnect() end
    if RXZ.amCircle then pcall(function() RXZ.amCircle:Destroy() end) end
    local part=Instance.new("Part")
    part.Name="RXZAutoMedusaRange"; part.Shape=Enum.PartType.Cylinder
    part.Size=Vector3.new(0.2,RXZ.AM_RANGE*2,RXZ.AM_RANGE*2)
    part.Color=Color3.fromRGB(255,255,255); part.Material=Enum.Material.Neon
    part.Transparency=0.6; part.Anchored=true; part.CanCollide=false; part.Parent=workspace
    RXZ.amCircle=part
    RXZ.amConn=RunService.Heartbeat:Connect(function()
        if not RXZ.autoMedusaOn then return end
        local ch=LP.Character; local root=ch and ch:FindFirstChild("HumanoidRootPart")
        if RXZ.amCircle and root then RXZ.amCircle.CFrame=CFrame.new(root.Position-Vector3.new(0,2.6,0))*CFrame.Angles(0,0,math.rad(90)) end
        if RXZ.isFrozen() then return end
        if not RXZ.enemyInRange() then RXZ.unequipMedusa(); return end
        local med=RXZ.findMedusaTool(); if not med then return end
        local hum=ch and ch:FindFirstChildOfClass("Humanoid")
        if hum and med.Parent~=ch then pcall(function() hum:EquipTool(med) end) end
        local held=ch and ch:FindFirstChildOfClass("Tool")
        if not held or RXZ.amBusy then return end
        RXZ.amBusy=true
        pcall(function() held:Activate() end)
        task.delay(RXZ.AM_CD,function() RXZ.amBusy=false end)
    end)
end
RXZ.stopAutoMedusa=function()
    RXZ.autoMedusaOn=false
    if RXZ.amConn then RXZ.amConn:Disconnect(); RXZ.amConn=nil end
    if RXZ.amCircle then pcall(function() RXZ.amCircle:Destroy() end); RXZ.amCircle=nil end
    RXZ.amBusy=false
    RXZ.unequipMedusa()
end
...
local function buildMobileButtons()
    ...
    if RXZ.autoMedusaBtn then
        local _, refAM = makeMobileBtn("AUTO\nMEDUSA", 0, 2, true, function(on)
            if on then RXZ.startAutoMedusa() else RXZ.stopAutoMedusa() end
            if RXZ.setAutoMedusaVisual then RXZ.setAutoMedusaVisual(on) end
        end)
        mobBtnRefs["autoMedusa"] = refAM
    end
    ...
    -- Sync stari curente
    if mobBtnRefs.autoMedusa then mobBtnRefs.autoMedusa(RXZ.autoMedusaOn) end
...
pcall(function()
    ...
    if type(d.autoMedusaBtn)=="boolean" then RXZ.autoMedusaBtn=d.autoMedusaBtn end
...
;(function()
    ...
    Outer.Name="Outer"; Outer.Size=UDim2.new(0,340,0,495); Outer.AnchorPoint=Vector2.new(0.5,0.5); Outer.Position=UDim2.new(0.5,0,0.5,0)
    Outer.BackgroundTransparency=1; Outer.BorderSizePixel=0; Outer.ClipsDescendants=false; Outer.Parent=GuiHub
    GuiRefs.outer=Outer
    -- compact scale for mobile (opens CENTERED on screen)
    ...
    MiniBtn.Size=UDim2.new(0,110,0,28); MiniBtn.AnchorPoint=Vector2.new(0.5,0); MiniBtn.Position=UDim2.new(0.5,0,0,124)
...
;(function()
    ...
    addSectLbl(cp,"ANTI MEDUSA",19)
    local _,svAntiMed=addToggleRow(cp,"Anti Medusa Counter",RXZ.antiMedusa,20,nil,function(on)
        RXZ.antiMedusa=on
...
;(function()
    ...
    local _,svAMBtn=addToggleRow(vi,"Auto Medusa Button",RXZ.autoMedusaBtn,18,nil,function(on)
        RXZ.autoMedusaBtn=on
        if not on then RXZ.stopAutoMedusa() end
        buildMobileButtons(); saveConfig()
    end)
    RXZ.setAutoMedusaVisual=svAMBtn
...
-- ============================================================
-- SPEED BYPASS (Velocity Y Trick) - RXZ HUB
-- ============================================================
do
    local RXZ_Players = game:GetService("Players")
    local RXZ_RunService = game:GetService("RunService")
    local RXZ_UIS = game:GetService("UserInputService")
    local RXZ_Player = RXZ_Players.LocalPlayer

    -- 1) Variables
    local VELOCITY_Y_TRICK = 0.000026
    local activeSpeedValue = 60      -- Change to your desired speed
    local isJumping = false
    local speedEnabled = true        -- Toggle on/off

    local Character = RXZ_Player.Character or RXZ_Player.CharacterAdded:Wait()
    local Humanoid = Character:WaitForChild("Humanoid")
    local HRP = Character:WaitForChild("HumanoidRootPart")

    -- 2) Jump Detection
    RXZ_UIS.InputBegan:Connect(function(input, gameProcessed)
    ...
    RXZ_RunService.Heartbeat:Connect(function()
    ...
    RXZ_RunService.Heartbeat:Connect(function()
        ...
        -- PRE-ROUND FREEZE: never fight the game's 5s hold before the round starts
        if RXZ.isFrozen() then
    ...
    -- 4) Character Reset
    RXZ_Player.CharacterAdded:Connect(function(char)
        Character = char
    ...
    -- Global API
    getgenv().RXZ_Speed = {
        set = function(v) activeSpeedValue = v end,
        toggle = function(v) speedEnabled = v end,
        get = function() return activeSpeedValue, speedEnabled end,
    }
end

print("RXZ HUB + SPEED BYPASS LOADED")
