...
;(function()
    ...
    Outer.Name="Outer"; Outer.Size=UDim2.new(0,340,0,495); Outer.AnchorPoint=Vector2.new(0,0); Outer.Position=UDim2.new(0,6,0,54)
    ...
    MiniBtn.Size=UDim2.new(0,110,0,28); MiniBtn.AnchorPoint=Vector2.new(0,0); MiniBtn.Position=UDim2.new(0,8,0,52)
...
;(function()
    ...
    addSectLbl(cp,"PROTECTION",0)
    local _,svAntiKick=addToggleRow(cp,"Anti Kick",RXZ.antiKick,1,nil,function(on)
    ...
    local _,svAutoMed=addToggleRow(cp,"Auto Medusa",RXZ.autoMedusaOn,4,nil,function(on)
        if on then RXZ.startAutoMedusa() else RXZ.stopAutoMedusa() end
        if mobBtnRefs.autoMedusa then mobBtnRefs.autoMedusa(on) end
        saveConfig()
    end)
    RXZ.setAutoMedusaVisual=svAutoMed
